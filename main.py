"""Demo: random binary votes, encrypted tally, double-vote rejection, voter receipts."""
...
def main(num_voters=10):
    ...
    ca = CA(bits=512)
    print(f"        CA public key n = {str(ca.pub['n'])[:30]}... ({ca.pub['n'].bit_length()} bits)")

    print("\n[setup] Generating election authority Paillier keys...")
    election = Election(ca, paillier_bits=512)
    print(f"        Paillier n = {str(election.pub['n'])[:30]}... ({election.pub['n'].bit_length()} bits)")

    print(f"\n[setup] Registering {num_voters} voters (each gets RSA keys + CA-signed cert)...")
    voters = [Voter(f"voter-{i:03d}", ca) for i in range(num_voters)]
    for v in voters:
        print(f"        {v.voter_id} registered, cert signed by CA")

    print("\n[vote ] Generating random binary votes and casting encrypted ballots...")
    ...
    print(f"\n        Cleartext votes (sanity only): {plaintext_votes}")
    print(f"        Expected Yes count: {expected_yes}")

    print("\n[atk  ] voter-000 tries to vote a second time (replay)...")
    ...
    print("\n[atk  ] outsider with a forged certificate (signed by a rogue CA) tries to vote...")
    ...
    print("\n[verify] Each voter re-encrypts with their saved randomness and checks the stored ballot...")
    for voter in voters:
    ...
    print("\n[tally] Homomorphically multiplying all ciphertexts and decrypting once...")
